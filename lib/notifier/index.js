...
const sendQywxBot = require('./vendor/qywx-bot')
...
class Notifier {
  constructor(options) {
    ...
    this.qywxWebhook = options.qywxWebhook
   ...
   * @return {Promise<String>}  推送结果
   */
  async sendQywxBot(title = '', content = '', {webhook}) {
    const qywxWebhook = webhook || this.qywxWebhook

    if (!qywxWebhook) {
      return {
        success: false,
        msg: `企业微信推送失败: 请设置 Webhook`
      }
    }

    return sendQywxBot({title, content, qywxWebhook})
  }
  ...
  notify(title, content, options = {}) {
    ...
    if (this.qywxWebhook) {
      result.push(this.sendQywxBot(title, content, options))
    }
